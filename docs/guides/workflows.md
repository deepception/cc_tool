...
The harness can now write and run its own multi-agent orchestration (the `Workflow` tool — triggered by the word "workflow", `/deep-research`, a saved workflow, or `ultracode` mode), spawning tens-to-hundreds of subagents whose intermediate results stay out of the main context. It is the mechanism for large independent fan-out. Every workflow agent runs on Opus 5.5 (cc_tool's settings force the subagent model), so stages are tuned with `effort`: `low` for routine arms, the session level or higher for plan/review stages (see `### Orchestration: which fan-out mechanism` in CLAUDE_snippet.md). Two things to know:
...
- **Run inside `cc-devcontainer`:** the firewall is the trust boundary, so widen `allow` in `settings.json` freely there and use `--permission-mode acceptEdits`. Note that `managed-settings.json` in the container *disables* `--dangerously-skip-permissions` (see [sandboxing](sandboxing.md)); a pre-resolved allowlist is the way to get a prompt-free run, not that flag.
