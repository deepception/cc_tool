...
Identify the surface of a claim BEFORE asserting it, and never transfer a claim across surfaces without evidence for the TARGET surface. An API parameter may have no harness equivalent, and the same-named feature can support different model sets on each surface (e.g. the API 'speed: "fast"' support matrix says nothing about which models the harness '/fast' command supports).`

const SIBLING_RULE = `ROOT-CAUSE SWEEP:
Before you write down a finding, check whether the same root cause hits sibling cases — grep/read for the pattern across the whole file, its sibling files, and any other place that repeats the construct (every model id in the same lookup table, every branch of the same conditional, every doc line stating the same rule). Enumerate every affected case in siblingCases and say in siblingSweepMethod exactly how you checked. A fix covers every instance of the defect, not the one you noticed first.`
...
const CONFIG_FOCUS = `Permission lists, sandboxing, and container config. Most of this is a security guardrail (secret reads, lockfile edits, push-to-main, install confirmation, firewall allowlist, blocking permission-skip flags) that protects regardless of how capable the model is. Flag a change only if a new model capability genuinely changes the risk calculus; a more capable model is not a reason to weaken a guardrail. Do check the mechanical side: does the config still reference files that exist, is every hook it registers present on disk, and does any model id / setting name in it match the current harness?`
...
- ${NEW_MODEL} may postdate your training data entirely, so coverage may be thin. Do not fill the gap from memory: omit the claim, or mark it 'speculative' and say so in the source field.
